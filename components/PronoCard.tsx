...
export function PronoCard({ p, hidePick }: { p: Prono; hidePick?: boolean }) {
  ...
  const state =
    p.result === "hit" ? "Gagné" : p.result === "miss" ? "Perdu" : p.result === "void" ? "Annulé" : "Ouvert";
  return (
    <Link className="card" href={`/pronostics/${p.id}`}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: ".6rem" }}>
        ...
        <span className={p.isPaid ? "badge pay" : "badge"}>{p.isPaid ? "Club" : "Public"}</span>
      </div>
      ...
      <div className="vs">
        <span>{locked ? "Réservé club" : p.pick}</span>
        {p.odd ? <span>{p.odd}</span> : null}
      </div>
      {p.stakeUnits ? <p className="muted">Mise {p.stakeUnits} u.{p.confidence ? ` · ${p.confidence}` : ""}</p> : null}
    </Link>
  );
}
